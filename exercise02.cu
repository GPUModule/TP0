...
int main(int argc, char *argv[])
{
	...
	/* Exercise 1.1: allocate device memory */
	//cudaMalloc(???);
	//cudaMalloc(???);
	...
	/* Exercise 1.2: copy host input to device input */
	//cudaMemcpy(???);
	checkCUDAError("Input transfer to device");

	/* Exercise 1.3: Configure the grid of thread blocks and run the GPU kernel */
	//dim3 blocksPerGrid(???);
	...
	/* Exercise 1.4: copy the gpu output back to the host */
	//cudaMemcpy(???);
	...
	/* Exercise 1.5: free device memory */
	//cudaFree(???);
	//cudaFree(???);
